...
export function leadKey(row, salt) {
  if (!String(salt || '').trim()) throw new Error('Sel d’empreinte manquant.');
  return sha256Hex(`${salt}|${directorFingerprintMaterial(row)}`);
}

export function personNameKey(row, salt) {
  if (!String(salt || '').trim()) throw new Error('Sel d’empreinte manquant.');
  return sha256Hex(`${salt}|${directorIdentity(row).nameMaterial}`);
}
