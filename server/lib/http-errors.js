...
export function edgeErrorStatus(error) {
  ...
  if (message === 'request_aborted') return 499;
  if (message === 'global_upstream_budget_exhausted') return 429;
  if (message === 'scan_deadline_exceeded' || message === 'server_busy') return 503;
  if (message.startsWith('government_api_') || message === 'upstream_invalid_payload' || message === 'upstream_failed') return 502;
