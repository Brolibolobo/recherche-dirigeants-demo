...
export function clampMaxRows(value) { return Math.min(1000, Math.max(1, Number.parseInt(value, 10) || 1)); }
...
export function buildReferenceRow(company, { apeLabel = '', exportedAt = new Date().toISOString(), sourceUrl = '', ageMin = 18, ageMax = 100, matchedEstablishment = null } = {}) {
  const siege = company.siege || {};
  const directors = company.dirigeants || [];
  const leader = directors.filter(d => physical(d) && d.nom && d.prenoms && !auditor(d)).filter(d => { const age = leaderAge(d); return age != null && age >= ageMin && age <= ageMax; }).sort((a,b) => rank(a)-rank(b))[0];
